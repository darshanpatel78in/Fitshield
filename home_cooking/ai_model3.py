...
from fitshield_webapp.AI.AIModel.model_helpers import nutrient_units, total_nutrients
...
def extract_ingredients(dish_name, description, ingredients):
    ...
    after_with = re.findall(r'\b\w+\b', description.split("with", 1)[0].strip().lower()) if "with" in description else re.findall(r'\b\w+\b', description.strip().lower())
...
def calculate_nutrients(ingredient_name, quantity_g, retriever):
    ...
    try:
        ...
        logger.error(f"Error invoking retriever for {ingredient_name}: {e}")
        return None
    if not simentic_search_result:
        logger.warning(f"No results found for {ingredient_name}")
        return None
...
def optimize_ingredient_categorization(dishdes_ingredients_list, ingredient_nutrient_categories, ingredient_quantity_categories):
    essential = set(dishdes_ingredients_list) if dishdes_ingredients_list else set()
    primary = set()
    secondary = set()
    flexible = set()

    # Essential Core Ingredients
    if "Essential - Core" in ingredient_quantity_categories:
        for i in ingredient_quantity_categories["Essential - Core"]:
            if i not in essential:
                primary.add(i)

    if "Essential - Core" in ingredient_nutrient_categories:
        for i in ingredient_nutrient_categories["Essential - Core"]:
            if i not in essential:
                primary.add(i)

    # Primary Ingredients
    if "Primary" in ingredient_quantity_categories:
        for i in ingredient_quantity_categories["Primary"]:
            if i not in essential and i not in primary:
                if i in ingredient_nutrient_categories.get("Primary", []):
                    primary.add(i)
                else:
                    secondary.add(i)

    if "Primary" in ingredient_nutrient_categories:
        for i in ingredient_nutrient_categories["Primary"]:
            if i not in essential and i not in primary:
                primary.add(i)  # Fix: Ensure it's added to primary

    # Secondary Ingredients
    if "Secondary" in ingredient_nutrient_categories:
        for i in ingredient_nutrient_categories["Secondary"]:
            if i not in essential and i not in primary:
                secondary.add(i)

    # Flexible Ingredients
    if "Flexible" in ingredient_nutrient_categories:
        for i in ingredient_nutrient_categories["Flexible"]:
            if i not in essential and i not in primary and i not in secondary:
                flexible.add(i)

    return list(essential), list(primary), list(secondary), list(flexible)
...
def categorize_ingredients_by_quantity(ingredients_list, serving_size):
    ...
    for item in ingredients_list:
        ...
        try:
            ...
            print(f"Error converting quantity_g to float: {e}")
            quantity_g_float = 5.0  # Assign default value as fallback

        print("Quantity in grams:", quantity_g_float)
...
def adjust_quantities_with_min_max(dish):
      print("before calculating min max")     #   print(json.dumps(dish, indent=4))
      ...
      print("after calculating min max")
